...
===
# 0. 앱 개요
Weight 앱은 내 무게의 추이를 한 눈에 볼 수 있는 앱입니다.

개인적으로 다이어트 성공을 위해 가장 중요했던 것이, 정말 시시때때로 내 몸무게를 체크하는 것이었습니다.
 
체중계에 올라가 몸무게를 사진 찍는 것만으로 내 몸무게 변동 추이를 확인할 수 있는 앱을 개발합니다.

iOS UI 구성, 개발 방법들에 대해 공부하며, 추가로 사진으로 몸무게를 인식할 수 있도록 머신러닝도 도입하여 개발할 예정입니다.
...
>주로 사용되는 개발 프로세스를 살펴보니 에자일 방법이 가장 괜찮게 느껴져서 해당 개발 프로세스를 채택하기로 결정했다.

---
# 2. 에자일 방법
- 보통 30일 단위로 스크럼 주기를 나누고, 짧게는 1-2주, 길게는 3-4주 단위의 스프린트를 쪼개서 개발한다.
- 스프린트는 개발 주기를 말한다
    - 특정 기간 동안 해야할 목표와 필요 작업을 명시하고, 실제로 어떻게 진행되었는지 Backlog를 남겨 각 스프린트가 끝나는 시점에 함께 모여 리뷰하고 피드백을 주고받는 형태로 업무를 진행.

- Github에서 제공하는 Issue 탭 + Milestone을 적절히 조합하여 각 스프린트 별로 일정을 정리하자.

---
# 3. 브랜치 관리
브랜치를 다음과 같이 크게 4가지로 관리하도록 한다.
> - Main branch
> - Feature branch
> - Release branch
> - Hotfix branch

Main branch가 프로젝트의 핵심 브랜치이며, 나머지 브랜치들은 Main branch에 merge가 되었다면 삭제하도록 하자.

## 3.1 Main branch
- Master branch
    - 배포 가능한 상태만을 관리하는 브랜치
    - 언제 어느 때든 배포가 가능한 상태이다.

- Develop branch
    - 다음 배포할 것을 개발하는 브랜치
    - 통합 브랜치의 역할을 담당. 이 브랜치를 기반으로 개발 진행


## 3.2 Supporting branch
- Feature branch
    - 기능을 개발하는 브랜치
    - develop 브랜치로부터 분기, 개발 완료 시 develop 브랜치로 merge
    - feature/ 로 명명한다.

- Release branch
    - develop 브랜치로부터 release 브랜치 생성
    - 배포를 위한 최종적인 버그 수정 등의 개발 수행
    - 배포 가능한 상태가 되면, master 브랜치로 병합, 출시 master branch에 버전 태그 추가
    - 배포 후 release 브랜치는 다시 develop branch로 병합


- Hotfix branch
    - 배포한 버전에서 긴급하게 수정을 해야할 필요가 있을 때, master 브랜치에서 분기
    - Hotfix 브랜치에서의 변경사항은 develop branch에도 merge 시켜주어야 한다.
---
# 4. 커밋 규칙
- 커밋 메시지 구조
